...
#verify if user wants to continue from last position
#if so, open file in append mode instead of creating and deleting the old one
if len(sys.argv) == 4:
	...
	for line in f_path:
		...
		while download_sucess == 0:
			try:
				...
				#save downloaded zip file in memory to avoid overusing Amazon EBS
				zip_inmemory = s3.get_object(Bucket='commoncrawl', Key=line.strip())
		...
		decompress_sucess = 0
		while decompress_sucess == 0:
			try:
				bytestream = io.BytesIO(zip_inmemory['Body'].read())
				gz = gzip.open(bytestream, mode='rt', encoding='latin-1')

			except Exception as e:
				print("Decompress error! Repeting decompress... " + file_name)
				print(str(e))
